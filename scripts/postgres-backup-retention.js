...
export function backupRetentionPolicy(env = process.env) {
  return {
    retentionDays: readNonNegativeInteger(env.OHADA_BACKUP_RETENTION_DAYS, 30, "OHADA_BACKUP_RETENTION_DAYS"),
    minCopies: readNonNegativeInteger(env.OHADA_BACKUP_MIN_COPIES, 7, "OHADA_BACKUP_MIN_COPIES")
  };
}

export async function prunePostgresBackups(directory, {
  retentionDays = 30,
  minCopies = 7,
  now = new Date()
} = {}) {
  ...
  const backups = await Promise.all(backupEntries.map(async (backup) => ({
    ...backup,
